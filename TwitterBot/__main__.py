...
def main():
    ...
    This dictionary is then passed to sentence which returns a finished sentence string
    '''
...
def generateTweet(sendingUser, chosenUser, tweetID, twitterInterface):
    ...
    tweets = twitterInterface.getTweets(chosenUser)
    if(tweets is None):
        return #do nothing
    ...
    classifier = classifyPoS(tweetBlob)
    tweetDict = classifier.createDictionary()
